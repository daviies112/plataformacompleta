...
export const ResidenceProofStep = (props: ResidenceProofStepProps = {}) => {
  ...
  const validateResidenceProof = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Erro na validação do comprovante');
      }
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Não foi possível validar o comprovante. Tente novamente.',
        variant: 'destructive'
      });
    } finally {
  ...
  const proceedWithMismatch = async () => {
    setIsProcessing(true);
    try {
      const contractId = window.location.pathname.match(/\/assinar\/([^/]+)/)?.[1];
      ...
      toast({
        title: 'Erro',
        description: 'Não foi possível salvar o comprovante. Tente novamente.',
        variant: 'destructive'
      });
    } finally {
      setIsProcessing(false);
