...
- **JWT Authentication Fix for Branding Page**: Fixed critical authentication chain preventing admin branding page from accessing Supabase credentials. Root causes: (1) `adminAuth.ts` JWT secret used only `JWT_SECRET` fallback but `authenticateConfig` used `JWT_SECRET || SESSION_SECRET` — mismatched secrets meant admin tokens were never accepted. Fixed by aligning `adminAuth.ts` secret chain to `JWT_SECRET || SESSION_SECRET || 'demo-secret...'`. (2) `multiTenantAuth.ts` login responses (dev bypass, direct login, Supabase Owner login) did NOT return JWT tokens — only created sessions. Fixed by adding `generateToken()` to ALL login response paths. (3) Added secure fallback Method 4 in `configAuth.ts`: accepts `x-tenant-id` header for GET requests ONLY when accompanied by a JWT token (even expired) whose decoded `tenantId` matches — handles server-restart scenarios where JWT_SECRET changes. Files: `server/services/adminAuth.ts`, `server/routes/multiTenantAuth.ts`, `server/middleware/configAuth.ts`.
...
- **Public Meeting URL Fix**: Fixed critical bug where `/reuniao-publica/slug/uuid` URLs failed because `PublicMeetingApp.tsx` URL parser only handled one-segment `/reuniao-publica/uuid` pattern. Added `twoSegmentPublicaPattern` regex to correctly extract the UUID when a company slug is present in the URL path.
- **Local Database Cache Cleanup System**: Implemented automatic cache cleanup for local PostgreSQL tables that duplicate Supabase data. Tables cleaned: forms (7d), form_submissions (7d), leads (7d), form_tenant_mapping (14d orphans), reunioes (30d), notification_history (14d). Before deleting, system verifies records exist in Supabase. Reunioes are synced to Supabase before cleanup. Never touches config tables (supabase_config, bigdatacorp_config, etc.) or wallets. Runs daily via scheduler. Manual trigger: `POST /api/credentials/cache-cleanup`. Status: `GET /api/credentials/cache-cleanup/status`. File: `server/lib/cacheCleanup.ts`.
- **Local DB Architecture Clarification**: 14 local PostgreSQL tables serve 3 purposes: (1) 7 config tables store service credentials, (2) 5 operational tables act as local cache of Supabase data for speed/fallback, (3) 2 tables (wallets, notification_history) are local-only operational data. Permanent data lives in Supabase; local DB is for fast access and resilience.
